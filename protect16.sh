...
echo "🚀 Install Proteksi Anti Modifikasi Detail Nodes 6..."

# Pastikan folder tujuan ada
mkdir -p "$(dirname "$REMOTE_PATH")"
chmod 755 "$(dirname "$REMOTE_PATH")"

# Tulis ulang file baru dengan proteksi ID 1
cat > "$REMOTE_PATH" <<'EOF'
<?php
...
class NodeViewController extends Controller
{
    ...
    /**
     * 🔒 Proteksi akses node hanya untuk admin ID = 1.
     */
    private function authorizeNodeAccess(): void
    {
        $user = Auth::user();

        // Jika tidak login, atau bukan admin utama (ID 1), blok akses
        if (!$user || $user->id !== 1) {
            throw new DisplayException('🚫 Akses ditolak: Anda tidak memiliki izin untuk melihat halaman node ini © Protect By Script Butterfly V 6.0.');
        }
    }
...
echo "✅ Install Proteksi Anti Modifikasi Detail Nodes 6 berhasil dipasang!"
